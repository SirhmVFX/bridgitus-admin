...
export async function POST(request: Request) {
  try {
    ...
    const docRef = await adminDb().collection("students").add({
      ...
      ...(source.paidAt ? { paidAt: source.paidAt } : {}),
      ...(source.planExpiresAt ? { planExpiresAt: source.planExpiresAt } : {}),
      ...(source.planQuota ? { planQuota: source.planQuota } : {}),
      ...(source.trialEndsAt ? { trialEndsAt: source.trialEndsAt } : {}),
      ...(source.trialStartedAt ? { trialStartedAt: source.trialStartedAt } : {}),
      trialUsed: Boolean(source.trialUsed || source.trialEndsAt),
