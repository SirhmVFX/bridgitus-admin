...
export default function PaymentsPage() {
  ...
  async function handleCreateAutoPay() {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: err instanceof Error ? err.message : "Failed to set up auto-pay",
      });
    } finally {
      setSubmitting(false);
    }
  ...
  async function handleCancelAutoPay(s: Student) {
    ...
    if (
      !confirm(
        `Cancel the ${s.autoPay?.interval} auto-pay for ${s.firstName} ${s.lastName}?`,
      )
    )
      return;
    ...
    try {
      ...
      setMessage({
        type: "ok",
        text: `Auto-pay cancelled for ${s.firstName} ${s.lastName}.`,
      });
      await load();
    } catch (err: unknown) {
      setMessage({
        type: "error",
        text: err instanceof Error ? err.message : "Failed to cancel auto-pay",
      });
    } finally {
  ...
  const stats = {
    ...
    expired: students.filter(
      (s) => s.paymentStatus === "expired" || s.paymentStatus === "failed",
    ).length,
  ...
  return (
    <AdminLayout>
      <div className=" mx-auto space-y-5">
        ...
        <p className="text-xs text-gray-400">
          Auto-pay uses the card saved when the student paid online through
          Stripe Checkout. Stripe automatically charges the card every week or
          month and retries failed charges. Students without a saved card must
          complete one online payment first.
        </p>
      </div>
          ...
          <div className="modal-box max-w-md">
            <div className="modal-header">
              <h2 className="font-semibold text-gray-900 flex items-center gap-2">
                <MdAutorenew size={18} className="text-[#00369b]" /> Set Up
                Automatic Payment
              </h2>
              <button
            ...
            <div className="p-6 space-y-4">
              <div className="bg-gray-50 border border-gray-200 p-3">
                ...
                {setupFor.stripePaymentMethod && (
                  <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                    ...
                    {setupFor.stripePaymentMethod.brand?.trim() || "Card"}{" "}
                    ending ····{setupFor.stripePaymentMethod.last4}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {(["weekly", "monthly"] as const).map((opt) => (
                    <button
                      key={opt}
                      onClick={() => setIntervalValue(opt)}
                      className={`py-2.5 text-sm font-semibold border-2 transition-colors capitalize ${
                        interval === opt
              ...
              <div>
                <label className="admin-label">
                  Amount per {interval === "weekly" ? "week" : "month"} (
                  {CURRENCY})
                </label>
                <input
                  type="number"
                ...
                {amount && !isNaN(parseFloat(amount)) && (
                  <p className="text-xs text-gray-400 mt-1">
                    The student's card will be charged{" "}
                    {formatAmount(Math.round(parseFloat(amount) * 100))} every{" "}
