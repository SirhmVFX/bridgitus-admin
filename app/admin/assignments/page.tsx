...
export default function AssignmentsPage() {
  ...
  async function load() {
    try {
      const [a, s, mats] = await Promise.all([
        getAllAssignments(),
        getAllStudents(),
        getAllMaterials(),
      ]);
...
${data.description ? `\n\n${data.description}` : ""}`,
        ...
        fetch("/api/notify-students", {
          ...
          body: JSON.stringify({
            ...
            studentIds: data.targetStudentIds?.length
  ...
  async function openLibrary(mode: "worksheet" | "quiz") {
    ...
    setLibModal(true);
    try {
      setQuestionSets(await getAllQuestionSets());
    } finally {
      setLibLoading(false);
    }
  }
  ...
  function importSetAsQuiz(set: QuestionSet) {
    ...
    setForm((f) => ({
      ...
      description:
        f.description || `${set.subject} — ${set.topic} (${set.difficulty})`,
  ...
  function importSetAsContent(set: QuestionSet) {
    ...
    set.questions.forEach((q, i) => {
      ...
      if (q.type === "multiple_choice" && q.options)
        ...
        lines.push(
          `<p>Answer: _______________________________________________</p>`,
        );
    });
    setForm((f) => ({
      ...f,
