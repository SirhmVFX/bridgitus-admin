...
export default function StudentsPage() {
    ...
    try {
      ...
      setLoadError(
        err instanceof Error
          ? err.message
          : "Failed to load data. Check your Firebase configuration and Firestore security rules.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleDelete(id: string, name?: string) {
    if (
      !confirm(
        `Delete ${name || "this student"}?\n\nThis removes their record and login so they can register again.`,
      )
    )
      return;
    setActionMsg(null);
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          throw new Error(
            data.message || data.error || `Delete failed (HTTP ${res.status})`,
          );
        }
      }
  ...
  const avgScore = (atts: TestAttempt[]) => {
    ...
    return Math.round(
      approved.reduce((s, a) => s + a.percentage, 0) / approved.length,
    );
  };
            ...
            <div className="p-12 text-center">
              ...
              <div className="mt-4 p-4 bg-amber-50 border border-amber-200 text-xs text-amber-700 text-left max-w-lg mx-auto">
                ...
                <ul className="list-disc pl-4 space-y-1">
                  <li>
                    Ensure you are logged in as an admin user registered in
                    Firestore (/admins collection)
                  </li>
                  <li>
                    Check your Firestore security rules allow authenticated
                    reads on the &apos;students&apos; collection
                  </li>
                  <li>
                    Verify your Firebase env vars in .env.local are correct
                  </li>
                </ul>
              </div>
            </div>
            ...
            >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1 flex-wrap">
                    ...
                    <span
                      className={`badge ${viewStudent.status === "active" ? "badge-green" : "badge-gray"}`}
                    >
                      {viewStudent.status}
                    </span>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <p className="font-mono font-bold text-[#00369b]">
                      {viewStudent.studentId}
                    </p>
                  </div>
                  <div>
                ...
                <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
                  <MdEmail size={12} /> Credentials were emailed at
                  registration.
                </p>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <p className="font-medium">
                      {viewStudent.parentFirstName} {viewStudent.parentLastName}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Phone</p>
                    <p className="font-medium">{viewStudent.parentPhone}</p>
                  </div>
                  <div className="col-span-2">
                    <p className="text-xs text-gray-400">Email</p>
                    <p className="font-medium">{viewStudent.parentEmail}</p>
                  </div>
                </div>
              </div>
              ...
              {detailLoading ? (
                ...
                <>
                        ...
                        {studentAttempts.map((a) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {a.status === "approved" && (
                                <span
                                  className={`font-bold ${a.passed ? "text-emerald-600" : "text-red-500"}`}
                                >
                                  {a.percentage}%
                                </span>
                              )}
                              <span
                                className={`badge ${a.status === "approved" ? (a.passed ? "badge-green" : "badge-red") : "badge-yellow"}`}
                              >
                                {a.status.replace("_", " ")}
                              </span>
                            </div>
                  ...
                  {/* Materials completed */}
                  {(() => {
                    const gradeM = allMaterials.filter(
                      (m) => m.grade === viewStudent?.grade,
                    );
                    if (gradeM.length === 0) return null;
                    const completedIds = new Set(
                      studentCompletions.map((c) => c.materialId),
                    );
                    const doneCount = gradeM.filter((m) =>
                      ...
                      <div>
                        ...
                        <div className="h-2 bg-gray-100 mb-3">
                          <div
                            ...
                            style={{
                              width: `${gradeM.length > 0 ? Math.round((doneCount / gradeM.length) * 100) : 0}%`,
                            }}
                          />
                        </div>
                        <div className="space-y-1 max-h-40 overflow-y-auto">
                          {gradeM.map((m) => (
                            <div
                              key={m.id}
                              className={`flex items-center justify-between text-sm px-3 py-1.5 ${completedIds.has(m.id!) ? "bg-emerald-50" : "bg-gray-50"}`}
                            >
                              <span
                                className={
                                  completedIds.has(m.id!)
                              ...
                              <span
                                className={`text-xs font-semibold ${completedIds.has(m.id!) ? "text-emerald-600" : "text-gray-400"}`}
                              >
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  value={statusEdit}
                  onChange={(e) =>
                    setStatusEdit(e.target.value as Student["status"])
                  }
                  className="admin-input"
                >
                  <option value="active">Active</option>
